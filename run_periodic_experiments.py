...
def build_experiments(output_root: Path) -> list[PeriodicExperiment]:
    return [
        PeriodicExperiment(
            ...
            output_dir=output_root / "rms_basic",
            enable_memory_penalty=False,
        ),
        PeriodicExperiment(
            name="rms_memory_penalty_factor_0_25",
            workload_path=Path("data/periodic_tasks_basic.json"),
            output_dir=output_root / "rms_memory_penalty_factor_0_25",
            enable_memory_penalty=True,
            memory_penalty_factor=0.25,
        ),
        PeriodicExperiment(
            name="rms_memory_penalty_factor_0_5",
            workload_path=Path("data/periodic_tasks_basic.json"),
            output_dir=output_root / "rms_memory_penalty_factor_0_5",
            enable_memory_penalty=True,
            memory_penalty_factor=0.5,
        ),
        PeriodicExperiment(
            name="rms_memory_penalty_factor_1_0",
            workload_path=Path("data/periodic_tasks_basic.json"),
            output_dir=output_root / "rms_memory_penalty_factor_1_0",
            enable_memory_penalty=True,
